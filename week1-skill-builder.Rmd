...
```{r martians}
#the probability of land is the probability of earth*pr(land+earth) + probability of mars*pr(land +mars)
pr_earth<- 0.5
pr_mars<- 0.5
pr_land_earth<- 0.3
pr_land_mars<- 1
...
```{r more-land}
#after understanding the earlier chunk I can tell that the dbinom controls the water variance, so that makes this:
p_grid <- seq( from=0 , to=1 , length.out=100 )
...
```{r 3M1}
p_grid <- seq( from=0 , to=1 , length.out=1000 ) # hypotheses
prior <- rep( 1 , 1000 )
tosses <- 9 # data
water <- 6 # data
